...
export class BudgetService {
  ...
  getAll(): Observable<Budget[]> {
    return this.http.get<any[]>(this.apiUrl).pipe(map(bs => bs.map(b => this.normalize(b))));
  }
  ...
  create(budget: any): Observable<Budget> {
    return this.http.post<any>(this.apiUrl, budget).pipe(map(b => this.normalize(b)));
  }

  update(id: string, budget: any): Observable<Budget> {
    return this.http.put<any>(`${this.apiUrl}/${id}`, budget).pipe(map(b => this.normalize(b)));
  }
